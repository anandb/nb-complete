...
[![Version](https://img.shields.io/badge/version-1.2.20-blue.svg)](pom.xml)
...
The **ACP NetBeans Plugin** is a state-of-the-art AI coding assistant integrated directly into the NetBeans IDE. It implements the **Agent Client Protocol (ACP)** to provide a seamless, real-time interactive chat experience for developers—supporting code generation, explanation, and project-aware assistance.
...
- **Collapsible Blocks**: Automatically handles "Thought" and "Code" segments. Use global toggles to expand or collapse all blocks in the conversation.
- **Markdown Rendering**: Rich text formatting with syntax highlighting for code blocks using `RSyntaxTextArea`.
...
- **Local Persistence**: Custom titles and session metadata are stored in your NetBeans user root (`<user_dir>/opencode/`), ensuring they survive IDE restarts.
...
- **Standardized Options**: Configure the plugin via the official NetBeans Options dialog (**Tools -> Options -> ACP**).
...
2. Build the NBM package:
   ...
   ```
3. The plugin will be generated at `./target/nbm/acp-netbeans-plugin-1.2.20.nbm`.
...
Navigate to **Tools -> Options -> ACP** to configure the following:
...
- **ACPChatTopComponent**: The primary controller for the NetBeans window, managing state transitions between sessions.

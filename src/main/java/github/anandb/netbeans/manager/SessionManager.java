...
public class SessionManager implements SessionQuery, SessionControl {
    ...
    @Override
    public CompletableFuture<Void> setSessionModel(String sessionId, String modelId) {
        return rpcClient.setSessionModel(sessionId, modelId)
                .thenApply(res -> (Void) null)
                .whenComplete((res, ex) -> {
                    if (ex != null) {
     ...
     * the manual restart path (Restart Harness button, executable-path

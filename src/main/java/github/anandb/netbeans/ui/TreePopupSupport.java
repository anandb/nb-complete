 ...
 * <b>Copy</b> and <b>Send to Assistant</b> plus caller-supplied extra items,
 * and shows it.
 */
final class TreePopupSupport {
    ...
    static void showPopup(MouseEvent e, JTree tree,
        ...
        // Extract the text lazily on menu action, not while building the popup,
        // so right-clicking a huge subtree (e.g. a large test suite) never
        // freezes the EDT.
        menu.add(new AbstractAction("Copy") {
            @Override public void actionPerformed(ActionEvent ev) {
                Clipboard clip = Toolkit.getDefaultToolkit().getSystemClipboard();
                clip.setContents(new StringSelection(textBuilder.apply(selected)), null);
            }
        });
        menu.add(new AbstractAction("\uD83D\uDCAC Send to Assistant") {
            @Override public void actionPerformed(ActionEvent ev) {
                String text = textBuilder.apply(selected);
